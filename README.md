# Description
This is a reskin pack for [Hideous Destructor](https://codeberg.org/mc776/HideousDestructor) that changes all weapons that feature hands to show black gloves and no visible arms.
I did take some creative liberty and changed the colour of the Boss rifle, Revolver & Fire Axe, though these are mostly just leftovers from my personal pack that i decided to keep here.

There are also sprites for a handful of addons, such as:
- Peppergrinder
- Bangers & Mash
- RadTech Weapons
- Icarus Innovations
- Flintlocks from Cozi's Offworld Wares
- The Wyvern & Viper from Acecorp Engineering Dept.
- The MA75B & MA76C from WAN Industries
- A few standalone weapons & items

Warning: Since HDest and Weapon packs are still being developed, there is a chance that some sprites in this pack may break in the future. Even now, i may have missed a few sprites that might've been updated recently.
Feel free to point out any broken or outdated sprites.




# Credits
Matt for [Hideous Destructor](https://codeberg.org/mc776/HideousDestructor).

Melodica for [DoomGuy Bronto Hands](https://www.dropbox.com/scl/fi/jlo89k5kpu8rzggbkxnl3/mld_hdbrontohands.pk3?rlkey=yh15fjy19febu7v8qo0w2n4r4&e=1&st=wd5fa7gx&dl=0)

Everyone who worked on the weapon sprites for:

- [Peppergrinder](https://gitlab.com/hdiscord-saltmines/hd-peppergrinder)
- [Bangers & Mash](https://github.com/Potetobloke/PB_HDAddon_Bangers-And-Mash)
- [RadTech Weapons](https://git.gay/swampyrad/RadTechWeaponsPack)
- [Icarus Innovations](https://github.com/HDest-Community/icarus-innovations-improved)
- [Cozi's Offworld Wares](https://github.com/Deadondev/Cozis-Offworld-Wares)
- [WAN Industries](https://codeberg.org/Wanzer/HDst_WAN_Industries)
- [Acecorp Engineering Dept.](https://github.com/HDest-Community/ace-corp-extended)
